...
class _PainelPageState extends State<PainelPage> {
  ...
  Future<void> _loadUserDataAndSetupStream() async {
    ...
    print("PainelPage: Iniciando carregamento de dados...");
    try {
      final prefs = await SharedPreferences.getInstance();
      _fullUserName = prefs.getString('clienteNome');
      _displayUserName = _fullUserName?.split(' ').first ?? "Cliente";

      print("PainelPage: Nome completo carregado para query: $_fullUserName");
      print("PainelPage: Nome para exibição: $_displayUserName");

      if (_fullUserName != null && _fullUserName!.isNotEmpty) {
        print(
            "PainelPage: Configurando stream para coleção principal 'fazendas' onde 'cliente' == $_fullUserName");
        _fazendasStream = _firestore
            ...
            .map((snapshot) {
          print(
              "PainelPage: Recebido snapshot da coleção 'fazendas'. Docs: ${snapshot.docs.length}");
          if (snapshot.docs.isEmpty) {
            print(
                "PainelPage: Nenhum documento encontrado na coleção 'fazendas' para o cliente '$_fullUserName'.");
          }
          return snapshot.docs.map((doc) {
            print(
                "PainelPage: Mapeando doc fazenda: ${doc.id} -> ${doc.data()}");
            return Imovel.fromFirestore(doc);
          }).toList();
        }).handleError((error) {
          print("PainelPage: Erro no stream de fazendas: $error");
          // Retorna lista vazia em caso de erro para não quebrar o StreamBuilder
          return <Imovel>[];
        });
      } else {
        print(
            "PainelPage: Nome completo do cliente nulo ou vazio. Nenhuma fazenda será carregada.");
        _displayUserName = "Visitante";
        _fazendasStream = Stream.value([]); // Emite lista vazia
      }
    } catch (e) {
      print(
          "PainelPage: Erro ao carregar dados do SharedPreferences ou configurar stream: $e");
      ...
      if (mounted) {
        ...
        print("PainelPage: Carregamento inicial concluído.");
      }
    }
  }

  Future<void> _logout() async {
    print("PainelPage: Iniciando logout...");
    try {
      await _auth.signOut();
      final prefs = await SharedPreferences.getInstance();
      ...
      print(
          "PainelPage: Logout bem-sucedido. Navegando para Login (via AuthWrapper).");
      // O AuthWrapper cuidará da navegação
    } catch (e) {
      print("PainelPage: Erro ao fazer logout: $e");
      if (mounted) {
  ...
  void _navegarParaFazendaDepartamentos(String nomeFazenda) {
    if (_fullUserName == null) {
      print("Erro: Nome completo do cliente não disponível para navegação.");
      ScaffoldMessenger.of(context).showSnackBar(
