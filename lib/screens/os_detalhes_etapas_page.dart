...
import 'package:agrogeo_brasil_app/screens/os_chat.dart'; // Assumindo que esta tela existe
...
class _OsDetalhesEtapasPageState extends State<OsDetalhesEtapasPage> {
  ...
  Future<void> _mostrarDocumentosDaEtapa(EtapaComProgresso etapa) async {
    // Construir o caminho da pasta da etapa no Firebase Storage
    // clientes/NOME DO CLIENTE/fazendas/NOME DA FAZENDA/NOME DO SERVIÇO DATA (dd-mm-aaaa)/etapas/NOME DA ETAPA/
    String nomeServicoComData =
        "${widget.tipoServicoOs} ${_formatarDataParaPasta(widget.dataCriacaoOs)}";
    String nomeEtapaFormatado = etapa.nome
        .replaceAll(RegExp(r'[^a-zA-Z0-9_]+'), '_')
        .replaceAll(' ', '_'); // Normaliza o nome da etapa para o caminho
    ...
    developer.log('Tentando listar arquivos de: $caminhoStorage',
        name: 'OsDetalhesEtapasPage._mostrarDocumentosDaEtapa');

    try {
      final ListResult result =
          await FirebaseStorage.instance.ref(caminhoStorage).listAll();
      final List<Anexo> documentosDaEtapa = [];

      for (final Reference ref in result.items) {
        final String downloadUrl = await ref.getDownloadURL();
        final FullMetadata metadata = await ref.getMetadata();
        String tipo = metadata.contentType?.split('/').last ??
            ref.name.split('.').last ??
            'desconhecido';

        documentosDaEtapa.add(Anexo(
          nome: ref.name,
          url: downloadUrl,
          tipo: tipo,
        ));
      ...
      if (mounted) {
        if (documentosDaEtapa.isEmpty) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
                content: Text('Nenhum documento encontrado para esta etapa.')),
          );
        } else {
            ...
            builder: (BuildContext context) {
              return Container(
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    Text(
                      'Documentos da Etapa: ${etapa.nome}',
                    ...
                    Expanded(
                      child: ListView.builder(
                        itemCount: documentosDaEtapa.length,
                        itemBuilder: (context, index) {
                          final anexo = documentosDaEtapa[index];
                          return ListTile(
                            leading: Icon(_getIconForFileType(anexo.tipo)),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Erro ao buscar documentos da etapa: $e')),
        );
      }
    }
  }
                          ...
                          children: [
                            Row(
                              children: [
                                ...
                                // Novo botão para visualizar documentos da etapa
                                IconButton(
                                  icon: const Icon(Icons.folder_open,
                                      color: Colors.blueAccent),
                                  tooltip: 'Visualizar Documentos da Etapa',
                                  onPressed: () =>
                                      _mostrarDocumentosDaEtapa(etapa),
                                ),
                              ],
                            ),
